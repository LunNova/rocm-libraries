...
def parse_in_worker(logic_file, params):
    """Worker function for multiprocessing test."""
    print(f"  [Worker PID: {os.getpid()}] Parsing file...")
    ...
    print(f"  [Worker PID: {os.getpid()}] Parse complete, returning result")
    return result


def run_parse_test(logic_file, params, use_multiprocessing=False):
    ...
    try:
        if use_multiprocessing:
            print(f"[Main PID: {os.getpid()}] Starting multiprocessing worker...")
            with multiprocessing.Pool(processes=1) as pool:
                result_future = pool.apply_async(parse_in_worker, (logic_file, params))
                result = result_future.get()
            print(f"[Main PID: {os.getpid()}] Retrieved result from worker")
        else:
            result = LibraryIO.parseLibraryLogicFile(
                str(logic_file),
                params["assembler"],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Performance test for LibraryIO.parseLibraryLogicFile"
    )
    parser.add_argument(
        "--multiprocessing",
        "-m",
        action="store_true",
        help="Run parse in a multiprocessing worker to test pickle/unpickle behavior"
    )
